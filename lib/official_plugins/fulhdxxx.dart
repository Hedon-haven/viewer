...
class fullhdPlugin extends OfficialPlugin implements PluginInterface {
  @override
  final bool isOfficialPlugin = true;
  @override
  String codeName = "fullhd-official";
  @override
  String prettyName = "fullhd.xxx";
  @override
  Uri iconUrl = Uri.parse("https://www.fullhd.xxx/favicon.ico");
  @override
  String providerUrl = "https://www.fullhd.xxx";
  @override
  ...
  Map<String, dynamic> testingMap = {
    ...
    "testingVideos": [
      // This is the most watched video on fullhd (that is available in all regions)
  ...
  final String _videoEndpoint =
      "https://www.fullhd.xxx/view_video.php?viewkey=";
  final String _searchEndpoint = "https://www.fullhd.xxx/video/search?search=";

  final String _channelEndpoint = "https://www.fullhd.xxx/channels/";
  final String _modelEndpoint = "https://www.fullhd.xxx/model/";
  final String _pornstarEndpoint = "https://www.fullhd.xxx/pornstar/";
...
Future<Response> _performGetRequest(Uri uri, {Map<String, String>? headers}) async => await client.get(uri, headers: headers);
  @override
  Future<List<UniversalVideoPreview>> getHomePage(int page,
    ...
    // fullhd has a homepage and a separate page 1 video homepage
  ...
  Future<List<UniversalVideoPreview>> getSearchResults(
    ...
    String urlString = _searchEndpoint + encodedSearchString
        ...
        // fullhd considers 720p to be hd. No further narrowing is possible in the url
  ...
  Future<List<UniversalVideoPreview>> getVideoSuggestions(
      ...
      [void Function(String body)? debugCallback]) async {
    // fullhd doesn't allow loading more suggestions
    if (page > 1) {
      debugCallback?.call("fullhd doesn't allow loading more suggestions");
      return Future.value([]);
    }
  ...
  Future<UniversalVideoMetadata> getVideoMetadata(
    ...
    int? ratingsNegative;
    // fullhd removed the dislike counter
    ...
    // fullhd doesn't provide exact timestamps -> convert it
  ...
  Future<Uri?> getCommentUriFromID(String commentID, String videoID) {
    // fullhd doesn't have comment links
  ...
  Future<List<UniversalComment>> getComments(
            ...
            for (Element subChild in child.children) {
              if (subChild.className == "clearfix") {
                ...
                final repliesResponse = await _performGetRequest(
                    Uri.parse(
                        "https://www.fullhd.xxx${subChild.attributes["data-ajax-url"]!}"),
    ...
    // fullhd allows to get all comments in one go -> return empty list on second page
    if (page > 1) {
      debugCallback?.call(
          "fullhd allows to get all comments in one go -> return empty list on second page");
      return Future.value([]);
    ...
    Uri commentsUri = Uri.parse("https://www.fullhd.xxx/comment/show"
        "?id=$internalCommentsID"
        // not sure what exactly the upper limit is, but fullhd doesn't seem to throw an error
        "&limit=9999"
  ...
  Future<UniversalAuthorPage> getAuthorPage(String authorID,
    ...
    if (response.statusCode != 200) {
      ...
      // make sure fullhd didn't redirect to the all pornstars page
          ...
          for (Element element in pageHtml
            ...
            advancedDescription["Featured in ${element.text.trim()}"] =
                element.attributes["href"] != null
                    ? "https://www.fullhd.xxx${element.attributes["href"]!}"
                    : "";
          }
      ...
      if (links != null) {
        ...
        if (links.isNotEmpty) {
          ...
          externalLinks["Channel owner page"] = Uri.parse(
              "https://www.fullhd.xxx${links.last.attributes["href"]!}");
        }
      }
    } catch (e, stacktrace) {
